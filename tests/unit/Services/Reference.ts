...
import { Convert, TypescriptMetaModel } from '../../../app/Services/generated/TypescriptMetaModel';
...
describe('Reference', () => {
    ...
    it('should get entity', () => {
        ...
        const found: TypescriptMetaModel = reference.getEntity(metamodels[0].id) as any;
    ...
    it('should get entity name', () => {
        ...
        expect(reference.getEntityName(metamodels[0].id)).toEqual(metamodels[0].name);

        expect(() => reference.getEntityName(mock.id)).toThrowError(NotFoundException);
        expect(() => reference.getEntityName(mock.id)).toThrowError(`Property 'name' not found in entity`);
    });

    it('should get entity class basename', () => {
        repository.add(metamodels[0]);

        expect(() => reference.getEntityClassBaseName(metamodels[0].id)).toThrowError(NotFoundException);
        expect(() => reference.getEntityClassBaseName(metamodels[0].id)).toThrowError(
            `Property 'package' not found in entity`
        );

        expect(reference.getEntityClassBaseName(metamodels[0].classes[0].id)).toEqual(
            'Famix-Traits/TParameterizedTypeUser'
        );
    });
