...
describe("update command", () => {
  ...
  it("should run command to update metamodel from source", async () => {
    ...
    expect(fs.copy).toHaveBeenCalledWith("source", "destination", {
      overwrite: true,
    });
    expect(logger.info).toHaveBeenCalledWith(
      `Updated metamodel from source to destination`
    );
  });
  ...
  it("should run command to update the typescript interface from updated metamodel", async () => {
    ...
    expect(fs.readJson).toHaveBeenCalledWith("destination", {
      encoding: "utf8",
    });
    expect(logger.info).toHaveBeenCalledWith(
      `Updated [typescript]-[meta]-[model] interface`
    );
  });
});
